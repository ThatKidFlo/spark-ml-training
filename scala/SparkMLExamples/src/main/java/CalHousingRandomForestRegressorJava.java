 ...
 */
public class CalHousingRandomForestRegressorJava {
    public static JavaSparkContext sc;

    public static void main(String[] argv) {
        SparkConf sparkConf = new SparkConf();
        sparkConf.setAppName("RandomForestRegressorCaliforniaHousingData");
    ...
    public static JavaRDD<LabeledPoint> prepareData() {
        JavaRDD<String> allContent = sc.textFile("resources/cal_housing.csv");

        String header = allContent.first();

        JavaRDD<String> all = allContent.filter(s -> !s.equals(header));

        JavaRDD<HousingEntry> allDataset = all.map(x -> new HousingEntry(x));
    ...
    public static class HousingEntry {
        private final String line;

        public HousingEntry(String line) {
            this.line = line;
        }
